"""Generate diff and API reports for Warfare scripts.

This utility compares baseline runtime scripts against reference mods
(`old walo` and `gammas patch`) and outputs Markdown documentation under
the ``docs`` directory.
"""
...
GAMMA_WALO_DIR = ROOT / 'gamma_walo' / 'gamedata' / 'scripts'
DOCS_DIR = ROOT / 'docs'
...
def generate_diff(path_a: Path, path_b: Path, context: int = 3) -> list[str]:
    """Return unified diff between two files as a list of lines."""
    ...
    diff = difflib.unified_diff(
        a_lines,
        b_lines,
        fromfile=os.path.relpath(path_a, ROOT),
        tofile=os.path.relpath(path_b, ROOT),
        n=context,
    )
    return list(diff)
...
def create_runtime_vs_mod_report() -> None:
    """Generate a diff report for old walo, gamma patch and our mod."""

    report_lines = ["# Runtime vs Mod Diff Report", ""]
    for mod_dir, mod_name in [
        (GAMMA_WALO_DIR, "gamma_walo"),
        (OLD_WALO_DIR, "old_walo"),
        (GAMMA_PATCH_DIR, "gammas_patch"),
    ]:
        ...
        for fname in sorted(collect_files(mod_dir)):
            ...
            if not runtime_path.exists() or not mod_path.exists():
                continue
            diff_lines = generate_diff(runtime_path, mod_path)
            if diff_lines:
