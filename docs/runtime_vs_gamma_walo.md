...
| faction_expansions.script | 0 | 0 | keep |
...
| game_relations.script | 0 | 0 | keep |
...
| smart_terrain_warfare.script | 0 | 0 | keep |
...
| soulslike_scenarios.script | 23 | 3 | keep |

<details><summary>Diff for soulslike_scenarios.script</summary>
```diff
--- runtime files/gamedata/scripts/soulslike_scenarios.script
+++ gamma_walo/gamedata/scripts/soulslike_scenarios.script
@@ -1,3 +1,6 @@
+-- Modified by Codex: Crashlog Fix - soulslike scenario nil guard (2025-07-25)
+-- Added safety checks when loading saved state and transferring items
+
 local ignore_items = {
     bolt = "ignore_bolt",
...
-    else    
+        -- added nil check for older saves missing story table
+        if not self.logic_state.story then
+            self.logic_state.story = {
...
+    -- ensure story table exists during item transfer
+    self.logic_state.story = self.logic_state.story or {}
+
...
| squad_loot_recovery.script | 0 | 0 | keep |

<details><summary>Diff for squad_loot_recovery.script</summary>
```diff
```
</details>
...
+-- Modified by Codex: Crashlog Fix - tasks_defense.script (2025-07-26)
+-- Removed game_relations require and use global is_factions_enemies
...
+-- removed explicit require; is_factions_enemies is available globally
...
+        -- use global relation helper to avoid nil module
+        if is_factions_enemies(character_community(db.actor), "freedom") then
+                return false
+        end
...
| tasks_smart_control.script | 0 | 0 | keep |
...
| ui_pda_warfare_tab.script | 0 | 0 | keep |
...
| warfare.script | 0 | 0 | keep |

<details><summary>Diff for warfare.script</summary>
```diff
```
</details>
| warfare_factions.script | 0 | 0 | keep |
...
| warfare_options.script | 0 | 0 | keep |
